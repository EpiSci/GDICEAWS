...
class MA3PurePursuit(TMAObject):
    ...
    def selected(self, observation):
        ...
        self.inner.channels[0].setGains((1, 0.0001, 3))
        self.inner.channels[1].setGains((-.75, 0, -3))
    ...
    def compute_action(self, observation, info):
        ...
        tmpAltSetPoint = state[info[StateInfo.BANDIT_POSITION_H_SL_FT]] + 500
        ...
        if xy_range > 3000:
            ...
            tmpSpeedSetpoint = tmpSpeed - self._kts_to_fps(closure) # 0 closure to track
        ...
        # Limit pitch to point at bandit
        rel_alt = state[info[StateInfo.BANDIT_POSITION_H_SL_FT]] - state[info[StateInfo.SELF_POSITION_H_SL_FT]]
        if tmpPitchSetPoint > 0:
            tmpPitchSetPoint = min(tmpPitchSetPoint, math.asin(rel_alt / dist) - math.radians(5))
        else:
            tmpPitchSetPoint = max(tmpPitchSetPoint, math.asin(rel_alt / dist) + math.radians(5))
        ...
        if state[info[StateInfo.SELF_SCORE_FLOOR_HEIGHT]] < 2500 and math.degrees(state[info[StateInfo.SELF_ATTITUDE_PITCH_RAD]]) < 5:
            tmpAileron /= 2
            tmpElevator = min(tmpElevator/2, -0.25)
            tmpRudder /= 5
            tmpThrottle /= 2
