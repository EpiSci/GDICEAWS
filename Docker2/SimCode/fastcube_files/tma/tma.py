...
class TMAObject(ABC):
    ...
    def _calculate_closure_kts(self, distance):
        if not self.prev_distance:
            self.prev_distance = distance
            return 0

        closure = self._fps_to_kts((self.prev_distance - distance) / .02)
    ...
    def _get_macro_observation(self, curr_ma, angle_off, aspect_angle, state, info):
        ...
        closing_velocity = self._calculate_closure_kts(tct_range)
        self.prev_distance = tct_range
